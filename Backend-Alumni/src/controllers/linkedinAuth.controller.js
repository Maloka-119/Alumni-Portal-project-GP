...
const LINKEDIN_REDIRECT_URI =
  ...
  "/alumni-portal/auth/linkedin/callback";
...
const handleLinkedInCallback = asyncHandler(async (req, res) => {
  try {
      ...
      if (nationalIdFromSession && validateNationalId(nationalIdFromSession)) {
        ...
        if (storedNID && storedNID !== nationalIdFromSession) {
          ...
          return res.redirect(
            "http://localhost:3000/login?error=" +
              encodeURIComponent(
                "This LinkedIn account is already linked to another person's graduation data. If you want to use a different National ID, please log out of LinkedIn and sign in with a different LinkedIn account.",
              ),
          );
        }
      }
    ...
    try {
      logger.info("Checking Staff API", {
        nationalId: nationalIdFromSession.substring(0, 5) + "...",
      });
      ...
      logger.warn("Staff API check failed or returned error", {
        error: e.message,
      });
    }
    ...
    if (!foundInAPI) {
      try {
        logger.info("Checking Graduate API", {
          nationalId: nationalIdFromSession.substring(0, 5) + "...",
        });
        ...
        if (facultyField) {
          logger.info("User found in Graduate API", {
            email,
        ...
        logger.warn("Graduate API check failed or returned error", {
          error: e.message,
        });
      }
    }
