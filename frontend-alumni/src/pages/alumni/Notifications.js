...
const NotificationsPage = ({ openChat, setUnreadCount }) => {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await axios.get("/alumni-portal/notifications", {
  ...
  const markAsRead = async (id) => {
    try {
      await axios.put(
        `/alumni-portal/notifications/${id}/read`,
        {},
        { headers: getAuthHeaders() },
      );
  ...
  const markAllAsRead = async () => {
    try {
      await axios.put(
        "/alumni-portal/notifications/read-all",
        {},
        { headers: getAuthHeaders() },
      );
  ...
  const deleteNotification = async (id) => {
    try {
      await axios.delete(`/alumni-portal/notifications/${id}`, {
        headers: getAuthHeaders(),
      });
  ...
  const fetchInvitations = async () => {
    try {
      const res = await axios.get("/alumni-portal/invitations/received", {
        headers: getAuthHeaders(),
      });
...
useEffect(() => {
  ...
  const acceptInvitation = async (invitationId) => {
    ...
    try {
      await axios.post(
        `/alumni-portal/invitations/${invitationId}/accept`,
        {},
        { headers: getAuthHeaders() },
      );
      setInvitations((prev) =>
  ...
  const rejectInvitation = async (invitationId) => {
    if (!invitationId) return console.error("Invitation id is missing");
    try {
      await axios.delete(`/alumni-portal/invitations/${invitationId}`, {
        headers: getAuthHeaders(),
      });
      setInvitations((prev) =>
        prev.filter((inv) => inv.invitationId !== invitationId),
