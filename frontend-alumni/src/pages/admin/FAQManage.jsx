...
function FAQManage({ currentUser }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewFAQ({ question_ar: "", question_en: "", answer_ar: "", answer_en: "" });
      setEditId(null);
      setShowForm(false);
      await fetchFaqs();
    } catch (err) {
          ...
          faqs.map((faq) => (
            <div key={faq.faq_id} className={`faq-item ${openId === faq.faq_id ? "open" : ""}`}>
              <div className="faq-header">
                <h4>{i18n.language === "ar" ? faq.question_ar : faq.question_en}</h4>
              ...
              <div className={`faq-answer ${openId === faq.faq_id ? "open" : ""}`}>
                <p>{i18n.language === "ar" ? faq.answer_ar : faq.answer_en}</p>
              </div>
            </div>
          ))
        ) : (
