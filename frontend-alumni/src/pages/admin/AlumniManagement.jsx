...
const AlumniManagement = ({ currentUser }) => {
  const [users, setUsers] = useState([]);
  const [selectedUser, setSelectedUser] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  ...
  const perms = currentUser?.userType === "admin"
    ...
    : getPermission("Graduate management", currentUser) || { canView: false, canAdd: false, canEdit: false, canDelete: false };
  ...
  useEffect(() => {
    if (!perms.canView || activeTab !== "inPortal") return;
    setLoading(true);
  ...
  {currentUser?.userType === "admin" && (
    ...
    >
      ...
      {activeTab === "inPortal" && perms.canView && (
        <div className="table-container">
          <input
            type="text"
            placeholder={t("searchByNameOrId")}
...
{activeTab === "requests" && currentUser?.userType === "admin" && (
  <GraduateRequests currentUser={currentUser} />
)}
