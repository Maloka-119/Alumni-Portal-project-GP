...
const RolesManagement = () => {
  ...
  // دالة جديدة لجلب الموظفين المتاحين للرول
  ...
  // تحديث الـ useEffect ليجلب الموظفين المتاحين عند فتح المودال
  useEffect(() => {
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      ...
      if (res.data.status === "success") {
        ...
        // تحديث قائمة الموظفين المتاحين بعد الإضافة
        fetchAvailableStaff(roleObj.role_id);
        
        // تحديث قائمة الموظفين الحاليين في الرول
        const staffRes = await API.get(`/roles/staff-by-role/${roleObj.role_id}`);
        const staffArray = staffRes.data.role.staff || [];
