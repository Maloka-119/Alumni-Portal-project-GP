...
function GroupsPage() {
  ...
  const openCreateForm = () => {
    if (showForm && !editingGroup) {
      // لو الفورم مفتوحة ومش في وضع التعديل، يقفلها
      setShowForm(false);
      cleanupPreview();
      setFormData({ name: "", description: "", cover: null });
      return;
    }
    // لو مش مفتوحة أو في وضع تعديل، تفتح فورم جديدة
    cleanupPreview();
    setFormData({ name: "", description: "", cover: null });
  ...
  return (
    <div className="grcontainer">
      <h1 style={{ color: "#4f46e5" }}>Communities</h1>
      ...
      <div className="controls">
        ...
        <button onClick={openCreateForm}>
          {showForm && !editingGroup ? "Cancel" : "Create Community"}
        </button>
      </div>
...
//           onChange={(e) => setSearch(e.target.value)}
//         />
//         <button onClick={openCreateForm}>Create Community</button>
//       </div>

//       {showForm && (
//         <form className="form-card" onSubmit={handleSubmit}>
//           <h3 style={{color:"GrayText"}}>{editingGroup ? "Edit Group" : "Create New Group"}</h3>
